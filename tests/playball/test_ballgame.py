...
class TestBallGame:
    ...
    def test_add_ball(self, ballgame_1, count_store_all):
        ...
        assert count_store_all[0].text == "1 - 0"
    ...
    def test_send_tick_empty(self, ballgame_1, count_store_all):
        ballgame_1.needs_new_batter = [False, False]
        ballgame_1.state.balls = 3
        ballgame_1.send_tick()
        assert isinstance(count_store_all[0], GameState)
        assert count_store_all[0].balls == 3
        ballgame_1.state.balls = 4
        ballgame_1.send_tick()
        assert count_store_all[0].balls == 4
